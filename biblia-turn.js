...
function atualizarBotoes() {
        ...
        $('#scroll-left').toggleClass('hidden', current <= 1);
        $('#scroll-right').toggleClass('hidden', current >= total);
        $('#current-page-label').text(`Página ${current}`);
        $('#total-pages-label').text(`de ${total}`);
        updateDepth(flipbook, current);
    }
    
function updateDepth(book, newPage) {
    if (!book || !book.turn) return;
    var page = book.turn('page');
    var pages = book.turn('pages');
    if (!pages || pages === 0) return;
    
    // Use newPage ou page como fallback
    var currentPage = newPage !== undefined ? newPage : page;
    
    var depthWidth = 16 * Math.min(1, currentPage * 2 / pages);
    var frontSide = $('.ej-book .front-side .depth');
    if (frontSide.length) {
        if (currentPage > 3) {
            frontSide.css({ width: depthWidth + 'px', left: (20 - depthWidth) + 'px' });
        } else { 
            frontSide.css({ width: '0px' }); 
        }
    }
    
    depthWidth = 16 * Math.min(1, (pages - currentPage) * 2 / pages);
    var backSide = $('.ej-book .back-side .depth');
    if (backSide.length) {
        if (currentPage < pages - 3) {
            backSide.css({ width: depthWidth + 'px', right: (20 - depthWidth) + 'px' });
        } else { 
            backSide.css({ width: '0px' }); 
        }
    }
}
